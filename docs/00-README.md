...
For newcomers, read top-down: **Vision → Requirements → Architecture Principles & Invariants → Architecture Overview → Domain → Security (E2EE) → Data → Realtime → Protocol → Features → API → Infrastructure → Quality → Operations.** ADRs are read alongside the documents they govern. The build/write order is governed by `document-manifest.yaml`, not by this list.
